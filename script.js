...
function saveAddition() {
    ...
    task.status = "waiting leader correction";
    localStorage.setItem("taskList", JSON.stringify(taskList));
    closeAdditionModal();
    renderFilteredTasks();
}

function openLeaderCorrection(id) {
    const task = taskList.find(t => t.id === id);
    if (!task) return;
    currentTaskId = id;
    document.getElementById("addProductName-correction").value = task.addition.productName;
    document.getElementById("addQuantity-correction").value = task.addition.quantity;
    document.getElementById("addBatchNo-correction").value = task.addition.batchNo;
    document.getElementById("addExpiryDate-correction").value = task.addition.expiryDate;
    document.getElementById("addDateTime-correction").value = task.addition.dateTime;
    document.getElementById("addOperatorName-correction").value = task.addition.operatorName;
    // isi nama leader jika sudah ada
    document.getElementById("leaderNameInput").value = task.leaderName || '';
    document.getElementById("leader-correction-modal").style.display = "block";
}

function saveLeaderCorrection() {
    const task = taskList.find(t => t.id === currentTaskId);
    if (!task) return;

    task.addition.correction = {
        productName: document.getElementById("product-name-correction").checked,
        quantity: document.getElementById("quantity-correction").checked,
        batchNo: document.getElementById("batch-no-correction").checked,
        expiryDate: document.getElementById("expiry-date-correction").checked,
        dateTime: document.getElementById("date-time-correction").checked,
        operatorName: document.getElementById("operator-name-correction").checked
    };

    // simpan nama leader
    task.leaderName = document.getElementById("leaderNameInput").value.trim();

    task.status = "waiting for verify";
    localStorage.setItem("taskList", JSON.stringify(taskList));
    closeLeaderCorrectionModal();
    renderFilteredTasks();
}
function closeLeaderCorrectionModal() {
    document.getElementById("leader-correction-modal").style.display = "none";
}

function toggleCorrection(field) {
    // Fungsi ini dapat digunakan untuk mengaktifkan atau menonaktifkan field tertentu
...
function printTask(id) {
    ...
    if (task.type === "Replenish") {
        ...
        <style>
            ...
            .verify-cell { 
                text-align: center; 
                /* no background color for print clarity */
                font-weight: bold; 
                font-size: 18px;
                color: black;
            }
            ...
            <table>
                ...
                <tr>
                    <td>Product Name</td>
                    <td>${task.addition?.productName || "-"}</td>
                    <td class="verify-cell">${task.addition?.correction?.operatorName ? "&#10004;" : "&#9744;"}</td>
    <td>Correct</td>
                </tr>
                <tr>
                    <td>Quantity</td>
                    <td>${task.addition?.quantity || "-"}</td>
                    <td class="verify-cell">${task.addition?.correction?.operatorName ? "&#10004;" : "&#9744;"}</td>
    <td>Correct</td>
                </tr>
                <tr>
                    <td>Batch No.</td>
                    <td>${task.addition?.batchNo || "-"}</td>
                    <td class="verify-cell">${task.addition?.correction?.operatorName ? "&#10004;" : "&#9744;"}</td>
    <td>Correct</td>
                </tr>
                <tr>
                    <td>Expiry Date</td>
                    <td>${task.addition?.expiryDate || "-"}</td>
                    <td class="verify-cell">${task.addition?.correction?.operatorName ? "&#10004;" : "&#9744;"}</td>
    <td>Correct</td>
                </tr>
                <tr><td>Addition Date & Time</td><td>${task.addition?.dateTime || "-"}</td>
                <td>Verify by:</td>
                  <td>${task.leaderName || "-"}</td>
                </tr>
                <tr><td>Operator Stamp/Sign</td>
  <td>${task.addition?.operatorName || "-"}</td>
</tr>
...
function renderTasks(list = taskList) {
    ...
    list.forEach(task => {
        ...
        card.innerHTML = `
            ...
            ${task.status === "waiting leader correction" ?
                `<button class="btn-correction" onclick="openLeaderCorrection(${task.id})">Leader Correction</button>` : ""}
  </div>
        `;
